...
void createOrder(string inputId)
{
    ...
    while (true)
    {
		cout << "Enter product name to add to order (or type 'cancel' to cancel): ";
        cin >> productName;
        if (productName == "cancel")
        {
           break;
        }
...
void user_menu(string inputId)
{
    string choice;
    do
    {
        // Fetch the current name from the customers array each time
        string name = "";
        for (int i = 0; i < customer_count; i++)
        {
        ...
        if (name.empty())
        {
            name = "User"; // Fallback if not found
        }
